...
def main():
    ...
    parser.add_argument(
        "--channel", default="All", help="Channel filter: Browser, SIP, All"
    )
    parser.add_argument(
        "--outcome", default="All", help="Outcome filter: success, failed, All"
    )
    parser.add_argument(
        "--log_test", action="store_true", help="Flag to insert a test call record"
    )
    parser.add_argument(
        "--test_type", default="success", help="Test type: success or failed"
    )
    parser.add_argument(
        "--test_channel", default="Browser", help="Test channel: Browser or SIP"
    )
    ...
    if args.log_test:
        ...
        reason = (
            ...
            if is_succ
                ...
                args.test_reason
                if args.test_reason != "None"
                else "Incomplete Task / Early Hangup"
            )
        )
