...
def redact_sensitive_info(text: str) -> str:
    ...
    redacted = re.sub(
        r"(?i)\b(otp|pin|password|passcode|cvv)\s*[:=]?\s*\d+", r"\1 [REDACTED]", text
    )
...
def save_caller(user_id: str, name: str, language_preference: str, facts: dict) -> None:
    logger.info(
        f"Saving caller info for {user_id}: name={name}, lang={language_preference}"
    )
    try:
        ...
        for k, v in facts.items():
            if isinstance(v, str):
                ...
                clean_facts[k] = [
                    redact_sensitive_info(item) if isinstance(item, str) else item
                    for item in v
                ]
            else:
                clean_facts[k] = v
...
def log_outbound_call(
    ...
    logger.info(
        f"Logging outbound call in SQLite: {customer_name} ({phone_number}) | {outcome} | {duration_seconds:.1f}s"
    )
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
...
def create_escalation_record(
    ...
    norm_urgency = (
        urgency.capitalize()
        if urgency and urgency.capitalize() in valid_urgencies
        else "High"
    )

    logger.info(
        f"Creating escalation for user={user_id}, caller={caller_name}, category={reason_category}, urgency={norm_urgency}"
    )
    ...
    try:
        ...
        if existing:
            ref_id = existing[0]
            logger.info(
                f"Found existing open duplicate escalation {ref_id}. Updating ticket notes."
            )
...
def send_discord_webhook_notification(escalation: dict) -> None:
    ...
    if not webhook_url:
        logger.info(
            "No DISCORD_WEBHOOK_URL configured. Escalation saved to SQLite database locally."
        )
        return
    ...
    try:
        ...
        dup_tag = (
            " [UPDATED DUPLICATE TICKET]" if escalation.get("is_duplicate") else ""
        )
            ...
            "embeds": [
                {
                    ...
                    "fields": [
                        {
                            "name": "👤 Caller Name",
                            "value": escalation["caller_name"],
                            "inline": True,
                        },
                        {
                            "name": "📞 Follow-up Method",
                            "value": escalation["contact_method"],
                            "inline": True,
                        },
                        {
                            "name": "⚡ Urgency",
                            "value": escalation["urgency"],
                            "inline": True,
                        },
                        {
                            "name": "🌐 Language",
                            "value": escalation["caller_language"],
                            "inline": True,
                        },
                        {
                            "name": "📌 Status",
                        ...
                        {
                            "name": "📝 Issue Summary (Sanitized)",
                            "value": escalation["issue_summary"],
                            "inline": False,
                        },
                        {
                            "name": "🔍 Agent Steps Checked",
                            "value": escalation["steps_already_taken"],
                            "inline": False,
                        },
                    ],
                    "footer": {
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "Python-JanDhanSeva-Agent",
            },
        )
        with urllib.request.urlopen(req, timeout=4) as resp:
            logger.info(
                f"Successfully posted escalation {escalation['reference_id']} to Discord webhook. Status: {resp.status}"
            )
    except Exception as e:
...
def log_call_session(
    ...
    logger.info(
        f"Logging call session: {session_id} | Channel: {channel} | Outcome: {outcome} | Reason: {failure_reason} | Duration: {duration_seconds:.1f}s"
    )
...
def get_call_analytics(channel_filter: str = None, outcome_filter: str = None) -> dict:
    ...
    try:
        ...
        success_rate = (
            round((successful_calls / overall_total * 100), 1)
            if overall_total > 0
            else 0.0
        )
        ...
        # Average duration
        cursor.execute(
            f"SELECT AVG(duration_seconds) FROM call_logs{where_sql}", params
        )
        avg_dur_row = cursor.fetchone()
        avg_duration = (
            round(avg_dur_row[0], 1) if avg_dur_row and avg_dur_row[0] else 0.0
        )

        # Failure breakdown
        cursor.execute(
            f"SELECT failure_reason, COUNT(*) FROM call_logs{fail_sql} GROUP BY failure_reason",
            ch_params,
        )
        failure_breakdown = {
            row[0]: row[1] for row in cursor.fetchall() if row[0] and row[0] != "None"
        }
