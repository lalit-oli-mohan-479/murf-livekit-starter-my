...
def list_escalations(status_filter: str = None):
    ...
    print(f"\n{'=' * 70}")
    print(
        f"  ESCALATION TICKETS{' (Status: ' + status_filter + ')' if status_filter else ' (All)'}"
    )
    print(f"{'=' * 70}")
    ...
    for t in tickets:
        ...
        print(
            f"     Follow-up:  {t['contact_method']}  |  Language: {t['caller_language']}"
        )
...
def main():
    ...
    parser.add_argument(
        "--status",
        default="Resolved",
        help="New status: 'Open', 'In Progress', or 'Resolved' (default: Resolved)",
    )
    parser.add_argument(
        "--note", default="", help="Resolution note to append to the ticket"
    )
    parser.add_argument(
        "--list",
        action="store_true",
        dest="list_tickets",
        help="List all escalation tickets",
    )
    ...
    if not args.ref:
        parser.error(
            "--ref is required to update a ticket. Use --list to see all tickets."
        )
    ...
    updated = db.update_escalation_status(
        args.ref.strip().upper(), args.status, args.note
    )
    if not updated:
